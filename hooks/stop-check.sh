...
# `check` script and the tree has changes, run `bun run check` through check-once.sh and report
# the result. A tree that already passed (the agent ran check-once.sh itself, or an earlier stop
# did) costs nothing. It blocks the stop (exit 2) only when the check fails, so the agent reports
# the failure instead of stopping over a red tree. It never edits anything.
set -uo pipefail
