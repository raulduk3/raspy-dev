...
## Operate
Read [the platform map](platform.md) and, for any question about how the pieces fit, the activated release's `docs/how-it-connects.md` before acting. Observe live state first: rigs, seats, tmux, herdr spaces, the loop, usage and the transcripts involved. Say what exists, what is running and who owns it before changing anything. Prefer the platform's launchers and the `ai` menu's own paths; use raw `rig` where they do not reach, and say why. Design a team from the smallest set of seats, one folder per writer, with short guidance and no startup prompts unless asked. Validate a spec offline and with `--plan` before starting anything. Reversible local work on request proceeds; starting seats, activating releases, account, daemon and configuration changes follow the map's authority section. When the platform itself misbehaves, fix it at its source through a branch, and put a general lesson in `docs/how-it-connects.md` and a dated fact in memory.
