...
It is not a framework, a product, or a replacement for GitHub. It is the local glue around my editor, terminal, coding agents, repository templates, checks, and project workflow. The goal is simple: every tool can help, but GitHub remains the ledger and a human remains the release authority.
...
raspy-dev gives a shared contract to the tools I use day to day:
...
- **OpenRig**: a native project/control surface over the same repositories and loop state.
- **OpenClaw-style agents**: engineering and personal-assistant roles can read the same repository policy without becoming release authorities.
- **Bun, Node, Python, gh, git, jq, 1Password CLI**: the ordinary local toolchain used by the checks, helpers, and runbooks.
- **Laya**: optional local decision/probe service diagnostics through `ai-env doctor`.

Each tool keeps its native UX. This repo only supplies the common rules, scripts, templates, and adapters.
...
owner decision
  ...
  -> editor, terminal, Claude Code, Codex, OpenRig, or agent surface
  -> checks and guardrails
  -> pull request record
  -> human review, merge, tag, deploy
```

The important rule is that there is one ledger. Chats, agents, desktop apps, and terminals are work surfaces. GitHub issues and pull requests are the durable record.
...
~/Dev/raspy-dev or ~/Dev/dev-platform
  ...
  lib/ai_ecosystem/            session, workspace and environment adapters
